...
class Hand_Control:
    ...
    def run(self, hand_tracker, model, cv2_util, queue, lock):
        ...
        while True:
            ...
            time_threshold = self.my_parameters["time"] / 100
            same_hand_threshold = self.my_parameters["same_hand"] * 100
            landmark_skip_frame = max(self.my_parameters["skip_frame"], 1)
            start_recognizing_time_threshold = self.my_parameters["start_time"] / 10
            stop_recognizing_time_threshold = self.my_parameters["stop_time"] / 10
            multi_action_time_threshold = self.my_parameters["multi_time"]
            multi_action_cooltime = self.my_parameters["multi_cooltime"] / 10
            ...
            try:                    
                ...
                if not ok:
                    # if target == "control" and value == "on":
                    #     cv2_util.make_cam()
                ...
                if time.time() - state["prev_action"][1] < time_threshold * 2:
                    ...
                    # if state["prev_action"][0] == "select":
                    #     cv2_util.cap.release()
                    #     cv2.destroyAllWindows()
                    
                    if lock is not None and queue is not None:
                        lock.acquire()
                        queue.put(("gesture", str(state["prev_action"][0])))
                        lock.release()

                        cmd = state["prev_action"][0]
