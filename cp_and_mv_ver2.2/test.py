...
def create_trackbars(parameter):
    ...
    cv2.createTrackbar(
        "start_time", "gesture recognition", parameter["start_time"], 30, utils.nothing
    )
    cv2.createTrackbar(
        "stop_time", "gesture recognition", parameter["stop_time"], 30, utils.nothing
    )
    cv2.createTrackbar(
        "multi_time", "gesture recognition", parameter["multi_time"], 10, utils.nothing
    )
    cv2.createTrackbar(
        "multi_cooltime", "gesture recognition", parameter["multi_cooltime"], 10, utils.nothing,
    )
...
def save_current_parameters(parameters_dir, parameter):
    ...
    if res == "y":
        ...
        parameter["time"] = cv2.getTrackbarPos("time", "gesture recognition")
        parameter["same_hand"] = cv2.getTrackbarPos("same_hand", "gesture recognition")
        parameter["skip_frame"] = cv2.getTrackbarPos("skip_frame", "gesture recognition")
        parameter["start_time"] = cv2.getTrackbarPos("start_time", "gesture recognition")
        parameter["stop_time"] = cv2.getTrackbarPos("stop_time", "gesture recognition")
        parameter["multi_time"] = cv2.getTrackbarPos("multi_time", "gesture recognition")
        parameter["multi_cooltime"] = cv2.getTrackbarPos("multi_cooltime", "gesture recognition")
        print(f"save params: {parameter}")
